...
Invoke-RestMethod `
    ...
    -Headers @{ "X-Trace-Id" = "demo-trace-p1-001" } `
    -Body $body
```
...
Invoke-RestMethod `
    ...
    -Uri "http://127.0.0.1:8000/process" `
    -ContentType "application/json" `
    -Headers @{ "X-Trace-Id" = "demo-trace-p4-001" } `
    -Body $body
```
...
* Workflow metrics
* Mocked tool execution metrics

Example JSON log:

```json
{
  "timestamp": "2026-06-19T10:00:00.000000+00:00",
  "level": "INFO",
  "logger": "IncidentIQ.Core.RequestLoggingMiddleware",
  "message": "HTTP request completed",
  "trace_id": "demo-trace-p1-001",
  "http_method": "POST",
  "path": "/process",
  "status_code": 200,
  "duration_ms": 123.45
...
| -------------------------------- | ------------------------------------------------------------- |
| `POST /process`                  | `app/api/routes/process.py`                                   |
| `GET /health`                    | `app/api/routes/health.py`                                    |
| Resource retrieval endpoint      | `GET /incidents/{incident_id}`                                |
| At least 3 LangGraph agents      | Triage, Knowledge, Remediation, Notifier                      |
| Conditional routing              | `app/graph/workflow.py`                                       |
| RAG pipeline                     | `app/rag/`                                                    |
| ChromaDB persistent vector store | `app/rag/vector_store.py`                                     |
| Pydantic A2A contracts           | `app/schemas/agent_messages.py`                               |
| Structured output                | Triage Agent supports `.with_structured_output(TriageResult)` |
| Mocked tools                     | `app/tools/devops_tools.py`                                   |
| Dockerfile                       | `Dockerfile`                                                  |
| Docker Compose                   | `docker-compose.yml`                                          |
| GitHub Actions CI                | `.github/workflows/ci.yml`                                    |
| Structured logs                  | `app/core/logging_config.py`                                  |
| Trace ID                         | `app/core/trace.py`                                           |
...
### 3. Run P1 database incident

Use the P1 `/process` curl command.

Explain:

```text
This is a critical production database incident.
LangGraph routes it through Triage, Knowledge, Remediation, and Notifier.
The Remediation Agent invokes mocked DevOps tools.
```

### 4. Run P4 informational alert

Use the P4 `/process` curl command.

Explain:

```text
This is informational with no customer impact.
LangGraph skips Knowledge and Remediation and goes directly to Notifier.
```
...
```text
The system tracks HTTP metrics, incident severity counts, category counts,
workflow paths, and mocked tool executions.
```
